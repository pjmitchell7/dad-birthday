...
let unlockedAt=-100,pendingUnlockToast=false;const visited=new Set();try{for(const activity of JSON.parse(localStorage.getItem('dads-day-activities')||'[]'))if(['movie','gym','drive'].includes(activity))visited.add(activity);}catch{}
function syncUnlock(){const unlocked=visited.size===3;const btn=$('celebrate');if(btn){btn.hidden=!unlocked;btn.classList.toggle('locked',!unlocked);btn.classList.toggle('unlocked',unlocked);btn.disabled=!unlocked;btn.setAttribute('aria-label',unlocked?'Celebrate, unlocked birthday surprise':'Celebrate');}const prog=$('celebrate-progress');if(prog)prog.textContent='';}
function rememberActivity(mode){if(!['movie','gym','drive'].includes(mode))return;const previous=visited.size;visited.add(mode);try{localStorage.setItem('dads-day-activities',JSON.stringify([...visited]));}catch{}syncUnlock();if(previous<3&&visited.size===3){if(['idle','intro','selected'].includes(phase))unlockedAt=globalTime;else pendingUnlockToast=true;}}
syncUnlock();
...
function setControls(enabled){for(const el of document.querySelectorAll('.choice')){el.disabled=!enabled||(el.id==='celebrate'&&visited.size<3);if(el.id==='celebrate')el.hidden=visited.size<3;}$('choices').style.opacity=enabled?'1':'0';$('choices').style.pointerEvents=enabled?'auto':'none';}
