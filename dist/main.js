...
let unlockedAt=-100,pendingUnlockToast=false;
try{localStorage.removeItem('dads-day-activities');localStorage.removeItem('dads-day-activities-v2');localStorage.removeItem('dads-day-visited');sessionStorage.removeItem('dads-day-activities');}catch{}
const visited=new Set();
function syncUnlock(){const unlocked=visited.size>=3;const btn=$('celebrate');if(btn){btn.hidden=!unlocked;btn.disabled=!unlocked;btn.classList.toggle('locked',!unlocked);btn.classList.toggle('unlocked',unlocked);if(unlocked){btn.style.setProperty('display','flex','important');}else{btn.style.setProperty('display','none','important');}btn.setAttribute('aria-label',unlocked?'Celebrate, unlocked birthday surprise':'Celebrate');}const prog=$('celebrate-progress');if(prog)prog.textContent='';}
function rememberActivity(mode,deferredToast=false){if(!['movie','gym','drive'].includes(mode))return;const previous=visited.size;visited.add(mode);syncUnlock();if(previous<3&&visited.size>=3){if(!deferredToast&&['idle','intro'].includes(phase))unlockedAt=globalTime;else pendingUnlockToast=true;}}
syncUnlock();
...
function setControls(enabled){for(const el of document.querySelectorAll('.choice')){el.disabled=!enabled||(el.id==='celebrate'&&visited.size<3);}syncUnlock();$('choices').style.opacity=enabled?'1':'0';$('choices').style.pointerEvents=enabled?'auto':'none';}
function select(mode){if(!['intro','idle','selected'].includes(phase))return;if(mode==='celebrate'){celebrate();return;}if(!info[mode])return;selected=mode;phase='selected';phaseStart=globalTime;dad.group.visible=true;dad.group.position.copy(baseDad);dad.group.rotation.set(0,0,0);dad.setOutfit(mode);outfitStart=globalTime;home.doorPivot.rotation.y=0;home.doorwayGlow.intensity=mode==='movie'?3:0;home.entranceFrame.material.emissive.set('#efbd43');home.entranceFrame.material.emissiveIntensity=mode==='movie'?.5:0;for(const el of document.querySelectorAll('.choice'))el.setAttribute('aria-pressed',String(el.dataset.mode===mode));$('hint-text').textContent=info[mode].hint;$('destination-label').textContent=info[mode].target;$('destination').hidden=false;$('chapter').textContent=info[mode].chapter;layout();}
for(const el of document.querySelectorAll('.choice'))el.addEventListener('click',()=>select(el.dataset.mode));
function confirm(){if(phase!=='selected')return;const currentMode=selected;phase=currentMode==='movie'?'enter':'walk';phaseStart=globalTime;$('destination').hidden=true;setControls(false);$('hint-text').textContent=currentMode==='movie'?'Movie night.':currentMode==='gym'?'Time to work out.':'Let’s go.';if(currentMode&&['movie','gym','drive'].includes(currentMode))rememberActivity(currentMode,true);if(currentMode!=='movie')activeCar=currentMode==='gym'?silver:blue;else primeMovie();}
...
$('scene-home').addEventListener('click',()=>{if(selected&&['movie','gym','drive'].includes(selected))rememberActivity(selected,true);reset();});
...
function tick(){globalTime=(performance.now()-start)/1000;const frameDelta=Math.min(1,Math.max(0,globalTime-lastFrameTime));lastFrameTime=globalTime;let elapsed=globalTime-phaseStart,pose='idle',animationTime=globalTime;
 if(phase==='intro'){if(globalTime<1.5){home.doorPivot.rotation.y=-smooth(globalTime/.7)*1.5;dad.group.position.z=1.94+smooth(globalTime/1.5)*.96;pose='walk';}else if(globalTime<3.5){dad.group.position.copy(baseDad);pose='idle';home.doorPivot.rotation.y=-1.5*(1-smooth((globalTime-1.5)/1.2));}else if(globalTime<6.5){dad.group.position.copy(baseDad);pose='wave';home.doorPivot.rotation.y=0;}else{dad.group.position.copy(baseDad);phase='idle';phaseStart=globalTime;}}
