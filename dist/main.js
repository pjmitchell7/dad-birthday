...
function rememberActivity(mode){if(!['movie','gym','drive'].includes(mode))return;const previous=visited.size;visited.add(mode);syncUnlock();if(previous<3&&visited.size>=3){if(['idle','intro','selected'].includes(phase))unlockedAt=globalTime;else pendingUnlockToast=true;}}
syncUnlock();
...
$('scene-home').addEventListener('click',()=>{if(selected&&['cinema','gym-scene','highway'].includes(phase)){const elapsed=globalTime-phaseStart;if(selected==='movie'&&elapsed>=10)rememberActivity('movie');else if(selected==='gym'&&elapsed>=15)rememberActivity('gym');else if(selected==='drive'&&elapsed>=15)rememberActivity('drive');}reset();});
